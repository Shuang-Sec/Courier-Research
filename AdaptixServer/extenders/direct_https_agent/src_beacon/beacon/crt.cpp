// 文件作用：极简 C/C++ 运行时替代实现：在隐藏导入表/nostdlib 构建时提供 malloc、free、栈检查和 RTTI 桩。
#pragma once
#include <windows.h>

//
// Minimal CRT replacements for -nostdlib linking.
// Provides the small runtime helpers required for empty-IAT builds.
//
...
extern "C" {
    ...
    // crt_lstrcpynA 是极简字符串复制函数，用来复制 API 名称等短字符串。
    ...
    // crt_djb2 计算字符串 hash，给极简 API 解析使用。
...
#define HASH_RtlReAllocateHeap  0xbbc97911
    ...
    // resolveHeapFunctions 手动找到 HeapAlloc/HeapFree 等堆函数，供 malloc/free 使用。
...
// GCC/Clang/MinGW/Zig: emit __main stub for static constructor init
    // __main 是 mingw 可能需要的空初始化桩，避免链接失败。
    void __main(void) {}

#ifdef _WIN64
    // ___chkstk_ms 是栈检查桩，用于满足编译器生成的大栈帧检查调用。
...
#else // x86
    // __chkstk_ms 是 64 位常见的栈检查桩名称。
    NAKED_FUNC void __chkstk_ms(void)
    {
        asm volatile(
            "push   ecx\n"
            "push   eax\n"
...
// operator delete 的 size_t 版本把 C++ delete 转发到 free。
void operator delete(void* p, unsigned long long) noexcept { free(p); }
// operator delete 的 unsigned int 版本用于兼容不同 mingw 目标。
void operator delete(void* p, unsigned int) noexcept { free(p); }
// operator delete 的普通版本把对象释放转发到 free。
...
namespace std {
    ...
    // type_info 析构函数是 C++ RTTI 兼容桩，避免缺少运行时符号。
    type_info::~type_info() {}
}
...
namespace __cxxabiv1 {
    ...
    // __class_type_info 析构函数是 C++ RTTI 兼容桩。
    ...
    // __si_class_type_info 析构函数是 C++ RTTI 兼容桩。
