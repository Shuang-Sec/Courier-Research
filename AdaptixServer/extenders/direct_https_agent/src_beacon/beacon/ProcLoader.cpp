// 文件作用：手动模块/API 解析实现：通过遍历 PEB 和导出表，用 hash 找 DLL 和函数地址。
#include "ProcLoader.h"
#include "ApiLoader.h"
#include "utils.h"
#include "ntdll.h"

// Djb2A 计算 ASCII 字符串的 djb2 hash，用来匹配模块名或函数名。
...
// Djb2W 计算宽字符串的 djb2 hash，用来匹配 Unicode 模块名。
...
// GetModuleAddress 遍历 PEB 里的已加载模块，通过 hash 找到 DLL 基址。
...
// GetSymbolAddress 遍历 PE 导出表，通过函数名 hash 找到函数地址。
